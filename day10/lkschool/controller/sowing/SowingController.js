...
class SowingController {
     ...
     * 根据id去修改一条轮播图数据
     */
    editOneSowingInfo(req, res, next){
        // formidable 方式上传数据 **************************************************
        // 1. 创建实例
        const form = formidable({ multiples: true });
        // 2. 指定文件上传的目录
     ...
     * 加载轮播图列表页面
     */
    loadSowingListPage(req, res, next){
        //查询所有数据
     ...
     * 加载添加轮播图页面
     */
    loadAddSowingPage(req, res, next){
     ...
     * 加载修改轮播图页面
     */
    loadEditSowingPage(req, res, next){
